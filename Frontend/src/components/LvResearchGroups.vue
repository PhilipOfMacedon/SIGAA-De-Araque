			...
			<v-expansion-panel expand=true>
							...
							<v-flex>
								...
								<v-list>
									<v-btn round outline small color="info" @click="addNewMember">
										<v-icon center style="height:auto;">person_add</v-icon>
										Novo membro
									</v-btn>
								</v-list>
							</v-card-text>
					...
					<v-dialog v-model="newMember" max-width="600px">
						...
						<v-layout row>
							<v-flex text-xs-center xs6 md6>
								<v-btn large color="info" @click="add_confirm">
									Adicionar
								</v-btn>
							</v-flex>
							<v-flex text-xs-center xs6 md6>
								<v-btn large color="warning" @click="add_cancel">
									Cancelar
								</v-btn>
							</v-flex>
						</v-layout>
				...
				<v-dialog v-model="removeMember" max-width="600px">
					<v-card class="mx-auto px-3 pt-4 pb-3 black--tex card-register" color="white lighten-1" elevation=12>
						...
						<v-layout row>
							<v-flex text-xs-center xs6 md6>
								<v-btn large color="info" @click="remove_confirm">
									Confirmar
								</v-btn>
							</v-flex>
							<v-flex text-xs-center xs6 md6>
								<v-btn large color="warning" @click="remove_cancel">
									Cancelar
								</v-btn>
							</v-flex>
						</v-layout>
